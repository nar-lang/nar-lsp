...
import (
	"bytes"
	"fmt"
	"github.com/nar-lang/nar-lsp/internal/protocol/span"
	"go/ast"
	"go/token"
	"path/filepath"
	"sort"
...
func (m *Mapper) SpanRange(s span.Span) (Range, error) {
	...
	if !strings.EqualFold(filepath.Base(string(m.URI)), filepath.Base(string(s.URI()))) {
		return Range{}, fmt.Errorf("mapper is for file %q instead of %q", m.URI, s.URI())
	}
...
func (m *Mapper) PosLocation(tf *token.File, start, end token.Pos) (Location, error) {
	startOffset, endOffset, err := Offsets(tf, start, end)
	if err != nil {
		return Location{}, err
	}
